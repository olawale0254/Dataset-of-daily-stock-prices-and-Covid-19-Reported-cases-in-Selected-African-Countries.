...
```{r}
`NIG Stock During COVID-19` <- Data1$NGE
class(`NIG Stock During COVID-19`)
```
...
SA_During <- getSymbols(SA,
                   src="yahoo",
                   from="2020-03-01",
                   to="2020-09-30",
                   env=Data1)
`SA Stock During COVID-19` <- Data1$EZA
class(`SA Stock During COVID-19`)
...
EGPT_During <-getSymbols(EGPT,
                   src="yahoo",
                   from="2020-03-01",
                   to="2020-09-30",
                   env=Data2)
`EGPT Stock During COVID-19` <- Data2$EGY
class(`EGPT Stock During COVID-19`)
...
TAZ_During <-getSymbols(TAZ,
                   src="yahoo",
                   from="2020-03-01",
                   to="2020-09-30",
                   env=Data4)
`TAZ Stock During COVID-19` <- Data4$TRX
class(`TAZ Stock During COVID-19`)
...
GAB_During <-getSymbols(GAB,
                   src="yahoo",
                   from="2020-03-01",
                   to="2020-09-30",
                   env=Data3)
`GAB Stock During COVID-19` <- Data3$EC.PA
na.omit(`GAB Stock During COVID-19`)
class(`GAB Stock During COVID-19`)

```


```{r}
Pricee = as.xts(`NIG Stock During COVID-19`$NGE.Adjusted)
Price =log10(Pricee)
names(Price) = c("price")
`Nigeria Stock Returns During COVID-19` = dailyReturn(Price)
```

```{r}
Price1 = as.xts(`SA Stock During COVID-19`$EZA.Adjusted)
names(Price1) = c("price")
`South Africa Stock Returns During COVID-19` = dailyReturn(Price1)
```

```{r}
Price2 <- as.xts(`GAB Stock During COVID-19`$EC.PA.Adjusted)
names(Price2) <- c("price")
`Gabon Stock Returns During COVID-19` = dailyReturn(Price2)
```


```{r}
Price4 = as.xts(`TAZ Stock During COVID-19`$TRX.Adjusted)
```

```{r}
names(Price4) = c("price")
`Tanzania Stock Returns During COVID-19` = dailyReturn(Price4)
```

```{r}
Price5 <- as.xts(`EGPT Stock During COVID-19`$EGY.Adjusted)
names(Price5) <- c("price")
`Egypt Stock Returns During COVID-19` = dailyReturn(Price5)
...
```{r}
library(moments)
skewness(`Gabon Stock Returns During COVID-19`)
kurtosis(`Gabon Stock Returns During COVID-19`)
```

### Discriptive Statistics 
```{r}
plot(`Nigeria Stock Returns During COVID-19`)
plot(`South Africa Stock Returns During COVID-19`)
plot(`Gabon Stock Returns During COVID-19`)
plot(`Tanzania Stock Returns During COVID-19`)
plot(`Egypt Stock Returns During COVID-19`)
```
```{r}
library(pastecs)
options(scipen=100)
options(digits=2)
stat.desc(`Gabon Stock Returns During COVID-19`)
```
```{r}
adf.test(`Nigeria Stock Returns During COVID-19`)
adf.test(`South Africa Stock Returns During COVID-19`)
adf.test(`Gabon Stock Returns During COVID-19`)
adf.test(`Tanzania Stock Returns During COVID-19`)
adf.test(`Egypt Stock Returns During COVID-19`)
```


```{r}
Nig_be <- diff(`Nigeria Stock Returns During COVID-19`, differences = 1)
SA_be<-diff(`South Africa Stock Returns During COVID-19`, differences = 1)
Gab_be<-diff(`Gabon Stock Returns During COVID-19`, differences = 1)
Tan_be<-diff(`Tanzania Stock Returns During COVID-19`, differences = 1)
Egy_be<-diff(`Egypt Stock Returns During COVID-19`, differences = 1)
library(Hmisc)
#During
...
garchforecast2 <- ugarchforecast(spec, n.ahead = 100, n.roll = 5, data = `South Africa Stock Returns During COVID-19`, out.sample = 30)
...
garchforecast5 <- ugarchforecast(spec, n.ahead = 100, n.roll = 5, data = Egy_bef , out.sample = 10)

a <- plot(garchforecast5)
```
